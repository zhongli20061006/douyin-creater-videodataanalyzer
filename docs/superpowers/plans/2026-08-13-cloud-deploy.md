...
- [ ] **Step 1: 写失败测试**（新增 `tests/test_headless_config.py`）：验证中间件从 settings 读取 `PLAYWRIGHT_HEADLESS`，默认 True（云上安全），False 时保留有头；
- [ ] **Step 2: 实现**：`PlaywrightMiddleware` 与 `collector.fetch_author_videos_browser` 的 `headless=` 改为 `crawler.settings.get('PLAYWRIGHT_HEADLESS', True)` / 函数参数；
- [ ] **Step 3: 回归**：pytest 全量；
- [ ] **Step 4: Commit**。
...
- [ ] **Step 1: 写失败测试**：settings 在存在 local_config 的 MYSQL_* 时返回覆盖值；
- [ ] **Step 2: 实现**：settings.py 顶部 `from local_config import ... MYSQL_HOST/PORT/USER/DB`（缺失时保留默认）；
- [ ] **Step 3: 回归**：pytest 全量；
- [ ] **Step 4: Commit**。
...
- [ ] **Step 1: 写失败测试**：新增 `verify_read_guard`（复用 evaluate_write_guard，但 GET 也强制令牌；本机 Origin 白名单仍放行）；
- [ ] **Step 2: 实现**：给 `/api/videos`、`/api/videos/{id}`、`/api/stats`、`/api/analyze/*`、`/api/spider/log`、`/api/quality/export` 等 GET 加 `dependencies=[Depends(verify_read_guard)]`；
- [ ] **Step 3: 回归**：pytest 全量；前端 axios 需带令牌（若同源访问可豁免，见实现时决定）；
- [ ] **Step 4: Commit**。
...
- [ ] **Step 1: 写失败测试**（插件 options 默认值断言）；
- [ ] **Step 2: 实现**：默认后端 `http://47.120.36.73`（或做成可配置），manifest 加该地址，api.py ALLOWED_ORIGINS 加 `http://47.120.36.73`；
- [ ] **Step 3: 回归**：node --test、pytest；
- [ ] **Step 4: Commit**。
...
- [ ] 本地推送 master → 服务器 `git pull`（app 用户）；
- [ ] 用户提供抖音 Cookie → 写入服务器 `local_config.py`（600 权限，不提交）；
- [ ] 重启 `douyinpachong` 服务；
- [ ] 真机验证：面板鉴权、插件云上报（用户重新加载插件）、手动触发爬虫抓一条视频入库；
- [ ] 验证完成后更新交接文档并清理本计划分支。
