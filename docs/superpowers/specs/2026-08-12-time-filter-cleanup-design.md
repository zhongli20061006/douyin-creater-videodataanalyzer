...
2. **定时清理**：后端内置定时任务，每 30 天执行一次；**每次按 `update_time` 升序删除最旧的 200 条**（不是"全库只留 200 条"）；范围整个库；手动开关、默认关闭；开关状态存 Redis；参数（30 天/200 条）为后端集中常量；
3. **开关入口**：数据质量页 + 视频数据页各放一个开关（共用同一 Redis 开关，状态同步）；
...
- 执行：全库行数 > 200 时，按 `update_time` 升序取最旧 200 条 → 完整备份待删行 → `DELETE` → 更新 Redis `last_clean_time`；
- **行数 ≤ 200 时本次不执行**（记录日志说明跳过，不更新 `last_clean_time`，下一周期再检查）；
- 参数常量：`CLEANUP_INTERVAL_DAYS = 30`、`CLEANUP_BATCH_SIZE = 200`；
- Redis keys：`douyin:cleanup_enabled`（0/1）、`douyin:cleanup_last_time`（ISO 时间字符串）；
- 开关 API：`GET /api/cleanup/status`（读开关+上次执行时间）、`POST /api/cleanup/toggle`（写接口，走现有令牌守卫，body `{enabled: bool}`）；
- 备份目录：项目外默认路径（常量 `CLEANUP_BACKUP_DIR`，默认系统临时目录 `douyin_cleanup_backup`），文件名含时间戳；备份格式 CSV（video_id/标题/作者/互动/时间字段全集）。

### 4.2 边界与容错

- 后端重启：循环任务随进程重新注册，Redis 中的开关与上次执行时间不丢；
...
- 后端纯逻辑（`analyzer.py` / 新增 `cleanup_service.py`）：
  ...
  - 清理选择逻辑：升序取 200、不足 200 的行为、30 天到期判断、开关判断、备份先于删除；
- API 层（`tests/test_api_guard.py` 或新增）：三接口时间参数（正常过滤、缺省、非法 → 400）、toggle 接口守卫与开关切换；
- 前端：`npm run build` 验证三页选择器与开关（无单测基建）；
...
- 清理为破坏性操作：默认关闭；开启后每 30 天删最旧 200 条；删除前备份 + 日志；备份失败不删；
- "每 30 天"从上次执行时间起算（Redis 持久化），后端重启不重置；
- 全库行数 ≤ 200 时不执行清理（已确认）；行数 > 200 时每次删最旧 200 条；
